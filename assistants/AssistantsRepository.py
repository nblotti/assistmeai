...
class AssistantsRepository:
    INSERT_ASSISTANT_QUERY = ("INSERT INTO assistants (user_id,name, conversation_id,description,gpt_model_number) "
                              "VALUES (%s, %s, %s,%s ,%s) RETURNING id")
    UPDATE_ASSISTANT_QUERY = "UPDATE assistants set name = %s,description= %s, gpt_model_number= %s where id= %s"
    GET_ASSISTANT_BY_USER_QUERY = ("SELECT id::text AS id,user_id,conversation_id::text AS conversation_id,"
                                   "name,  description ,gpt_model_number from assistants where user_id=%s order by id ")
    GET_ASSISTANT_BY_CONVERSATION_ID_QUERY = ("SELECT id::text AS id,user_id,conversation_id::text AS conversation_id,"
                                              "name,  description, gpt_model_number from assistants where "
                                              "conversation_id=%s ")
    ...
    def update(self, assistant):
        ...
        cursor.execute(self.UPDATE_ASSISTANT_QUERY,
                       (assistant.name, assistant.description, assistant.gpt_model_number, assistant.id))
