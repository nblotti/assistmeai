...
class SqliteDAO:
    INSERT_PDF = "INSERT INTO document (id, name, userid, document) VALUES (?, ?, ?,? )"
    SELECT_DOCUMENT = "SELECT name, document FROM document WHERE id=?"
    LIST_PDF = "SELECT id, name, userid FROM document"
    DELETE_PDF = "DELETE FROM document where id = ?"
    DELETE_ALL= "DELETE FROM document;"

    def __init__(self):
        conn = sqlite3.connect('document.sqlite3')
